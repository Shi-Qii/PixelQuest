...
| 指令 | 功能 |
|---|---|
| `/setup` | 為當前關卡選題或自訂題目與單字 |

**進入新關卡時有三種模式：**
- `[1] 用預設` — Claude 精選的題目清單附難度標籤，直接開始
- `[2] 換掉幾題` — 保留大部分預設，用題號或描述替換（例如「把第 3 題換成 sliding window 的」）
- `[3] 完全自訂` — 說你想練的方向，Claude 推薦題目讓你選；或直接說題目名稱。單字：你說單字名，Claude 自動補詞性和例句

隨時可以重新呼叫 `/setup` 調整選題。已完成的題目不能移除，只能新增替補。
